...
(function(Scratch) {
  "use strict";
  if (!Scratch.extensions.unsandboxed) throw new Error("My Blocks+ 必须在非沙盒模式下运行!");
  ...
  let globalTargetFocus = undefined; // 与 globalBlocksCache 不同，这有助于在堆栈中查找积木ID
  ...
  function genDrpButton(container, opts, defaultValue, isDark, isLabel, changeFunc, setFunc) {
    ...
    // 在 Firefox 上，"click" 事件会传播到按钮，这会添加另一个输入。不要这样做
  ...
  function openMenuSelector(editor, isDark, ogWorkspace) {
    ...
    dropDiv.style.cssText = `
        ...
        max-height: 300px;
        overflow-y: auto;
        border: solid ${isDark ? "#343434" : "#D9D9D9"} 2px;
        border-radius: 10px;
        margin-bottom: 15px;
    `;

    // 选中的菜单值
    let selectedValue = allBlocks[0];

    // 创建菜单选项
    allBlocks.forEach((menuId, index) => {
        ...
        dropItem.style.cssText = `
            ...
            background: ${index % 2 === 0 ? (menuId.includes(CUSTOM_MENU_ID) ? "#cc526640" : "#aaa3") : "transparent"};
            color: ${isDark ? '#ffffff' : '#333333'};
        `;
        if (index === 0) {
            dropItem.style.backgroundColor = menuId.includes(CUSTOM_MENU_ID) ? "#f7889a99" : "#aaaa";
        }

        // 菜单预览
        const preview = document.createElement("div");
        const isCustom = menuId.includes(CUSTOM_MENU_ID);
        const block = ogWorkspace.getBlockById(menuId);
        let prevTxt = block ? (block.inputList?.[0]?.fieldRow?.[0]?.text_ || '').substring(0, 17) : "";
        if (prevTxt.length === 17) prevTxt += "...";
        
        preview.style.cssText = `
            color: #fff;
            background: ${isCustom ? "#FF6680" : block ? block.colour_ : "#505050"};
            border-radius: ${block ? 50 : 5}px;
            width: max-content;
            text-align: center;
            margin-bottom: 5px;
            padding: 5px 10px;
        ...
        `;
        preview.innerHTML = `
            <span>${block ? prevTxt : "???"}</span>
            <img style="margin-left: 5px; width: 12px; height: 8px;" src="${arrowIcon}.svg">
        `;

        // 菜单名称
        const text = document.createElement("div");
        text.textContent = menuId.replace(CUSTOM_MENU_ID, "");
        text.style.marginTop = "5px";

        dropItem.appendChild(preview);
        dropItem.appendChild(text);

        // 点击事件
        dropItem.addEventListener("click", () => {
            Array.from(dropDiv.children).forEach(c => {
                const isCustomItem = c.querySelector('div span').textContent.includes(CUSTOM_MENU_ID);
                c.style.backgroundColor = c === dropItem 
                    ? (isCustomItem ? "#f7889a99" : "#aaaa") 
                    : (c === dropItem ? (isCustomItem ? "#cc526640" : "#aaa3") : "transparent");
            });
    ...
    modalContainer.appendChild(dropDiv);
    ...
    confirmBtn.addEventListener('click', () => {
        ...
        const args = editor.argumentIds_;
        const id = args[args.length - 1];
        editor.displayNames_[args.length - 1] = selectedValue.replace(CUSTOM_MENU_ID, "");
        editor.updateDisplay_();
        editor.focusLastEditor_();

        const inputs = tempStore.inputs || {};
        inputs[id] = { type: selectedValue, isDrop: true };
        tempStore.inputs = inputs;
        cleanupBlockInputs(inputs, args);
  ...
  if (Scratch.gui) Scratch.gui.getBlockly().then(SB => {
    ...
    utils.updateDisplay_ = function() {
      ...
      // 在 PM 中更改输出类型时，断开原型与定义积木的连接并重新连接
      // 否则会发生错误
  ...
  function startBlockListener() {
    runtime.on("BLOCK_DRAG_END", (newStack, oldID) => {
      ...
      if (copyStore) setTimeout(() => {
        // 对于这个，timeout 比 queueMicrotask 效果更好？
        for (const target of runtime.targets) {
  ...
  function deserializeStorage(data) {
    if (isPM) {
      ...
      // 初始化全局积木可见性状态
      globalBlocksVisibility = data.SPmbpCST?.globalBlocksVisibility || {};
      startEditorListener();
      if (Scratch.gui) Scratch.gui.getBlockly().then(SB => {
        runtime.once("PROJECT_LOADED", () => {
      ...
      // 初始化全局积木可见性状态
      globalBlocksVisibility = runtime.extensionStorage["SPmbpCST"]?.globalBlocksVisibility || {};
  ...
  // 新增：获取全局积木菜单
  function getGlobalBlocksMenu() {
    const items = [];
    items.push({text: "显示全部", value: "show_all"});
    items.push({text: "隐藏全部", value: "hide_all"});
    items.push("---");
    
    // 获取所有有全局积木的角色
    const targetsWithGlobalBlocks = {};
    for (const [targetId, procs] of Object.entries(storage)) {
      if (typeof procs !== 'object') continue;
      
      const target = runtime.getTargetById(targetId);
      if (!target) continue;
      
      let hasGlobalBlocks = false;
      for (const [proccode, procData] of Object.entries(procs)) {
        if (procData.global) {
          hasGlobalBlocks = true;
          break;
        }
      }
      
      if (hasGlobalBlocks) {
        targetsWithGlobalBlocks[targetId] = target;
      }
    }
    
    // 为每个有全局积木的角色添加菜单项
    Object.entries(targetsWithGlobalBlocks).forEach(([targetId, target]) => {
      const isVisible = globalBlocksVisibility[targetId] !== false;
      items.push({
        text: `${isVisible ? "✓" : "✗"} ${target.getName()}`,
        value: `toggle_${targetId}`
      });
    });
  ...
  // 新增：切换全局积木可见性
  function toggleGlobalBlocksVisibility(targetId) {
    if (targetId === "show_all") {
      // 显示全部
      for (const id in globalBlocksVisibility) {
        globalBlocksVisibility[id] = true;
      }
    } else if (targetId === "hide_all") {
      // 隐藏全部
      for (const id in globalBlocksVisibility) {
        globalBlocksVisibility[id] = false;
      }
    } else if (targetId.startsWith("toggle_")) {
      // 切换单个角色的显示状态
      const actualTargetId = targetId.replace("toggle_", "");
      globalBlocksVisibility[actualTargetId] = !globalBlocksVisibility[actualTargetId];
    }
  ...
  function compileProcedures(xmlList, workspace, utils) {
    ...
    if (globalProcs.size > 0) {
      ...
      let hasVisibleGlobalBlocks = false;
      
      for (const proccode of globalProcs) {
        const target = globalBlocksCache[proccode];
        
        // 检查该角色的全局积木是否应该显示
        if (globalBlocksVisibility[target.id] === false) {
          continue; // 跳过隐藏的积木
        }
        
        hasVisibleGlobalBlocks = true;
      ...
      if (hasVisibleGlobalBlocks) {
        tempList += `<sep gap="6"/><label text="私有积木"/><sep gap="6"/>`;
        proceduresXML = tempList + proceduresXML;
      } else {
        // 如果没有可见的全局积木，直接显示私有积木
        tempList = `<sep gap="12"/><label text="私有积木"/><sep gap="6"/>`;
        proceduresXML = tempList + proceduresXML;
      }
    }
  ...
  vm._addSprite3 = function(...args) {
    ...
    // 如果存在，移除虚拟积木，这永远不会是舞台（保存的地方），所以不应该发生任何坏事
  ...
  class SPmbpCST {
    ...
    removeExt() {
      if (confirm("移除这个扩展？这将移除所有归因于我的积木+的新功能"))
        removeExtension();
    }
    ...
    // 新增：获取全局积木菜单
    getGlobalBlocksMenu() {
      return getGlobalBlocksMenu();
    }

    // 新增：管理全局积木显示
    manageGlobalBlocks(args) {
      toggleGlobalBlocksVisibility(args.MENU);
    }
      ...
      if (isPM) {
        ...
        return { 
          SPmbpCST: { 
            ...storage, 
            imgStorage,
            globalBlocksVisibility // 添加这一行
          } 
        };
      } else {
        runtime.extensionStorage["SPmbpCST"] = { 
          loaded: true, 
          imgStorage,
          globalBlocksVisibility // 添加这一行
        };
